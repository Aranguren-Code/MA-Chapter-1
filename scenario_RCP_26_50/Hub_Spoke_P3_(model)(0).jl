...
#HS_P3=Model(solver=GurobiSolver(Presolve=0; OutputFlag=0))
HS_P3=Model(solver=GurobiSolver(Presolve=-1, Cuts=0, MIPGap=0.001,
  OutputFlag=0))
...
@constraint(HS_P3, DEMAND_SUPPLY[j=1:C], sum(FLOWY[i, j] for i=1:D) +
  sum(FLOWZ[i, j] for i=1:P) + SHORTAGE[j] == Demand[j])
...
@objective(HS_P3, Min, sum((HarvCost[i] + TransCost1[i, j]) *
  ...
  sum(ShortCost[i]*SHORTAGE[i] for i=1:C))
